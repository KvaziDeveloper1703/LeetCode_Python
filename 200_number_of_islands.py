...
An island is a group of '1's (land) connected horizontally or vertically (not diagonally). All the edges of the grid are surrounded by water.
...
Необходимо вернуть количество островов на карте.

Остров — это группа ячеек '1', связанных по горизонтали или вертикали (но не по диагонали). Все края сетки окружены водой.
...
def number_is_lands(grid: List[List[str]]) -> int:
    if not grid:
        return 0
    total_rows = len(grid)
