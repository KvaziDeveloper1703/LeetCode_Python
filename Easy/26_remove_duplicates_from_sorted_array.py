...
Дан отсортированный по неубыванию массив целых чисел nums. Ваша задача — удалить дубликаты на месте, чтобы каждый уникальный элемент встречался только один раз, при этом сохраняя относительный порядок элементов.
После удаления дубликатов верните количество уникальных элементов — обозначим его как k.

Требования:
+ Измените массив nums на месте так, чтобы первые k элементов содержали уникальные значения в исходном порядке;
+ Остальная часть массива (начиная с индекса k) может содержать любые значения — она не имеет значения;
+ Верните значение k.
