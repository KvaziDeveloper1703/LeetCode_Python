'''
You are given n balloons, indexed from 0 to n - 1. Each balloon has a number on it, given in the array numbers.
Your task is to burst all the balloons in such an order that you maximize the total coins collected.
When you burst the i-th balloon, you will get numbers[i - 1] * numbers[i] * numbers[i + 1] coins.
...
Examples:
Input: numbers = [3,1,5,8]
Output: 167

Input: numbers = [1,5]
Output: 10

Вам даны n воздушных шариков, пронумерованных от 0 до n - 1. На каждом шарике написано число — они представлены в массиве numbers.
Ваша задача — лопнуть все шарики в таком порядке, чтобы максимизировать количество полученных монет.
Когда вы лопаете i-й шарик, вы получаете numbers[i - 1] * numbers[i] * numbers[i + 1] монет.
...
def max_coins(balloon_values: List[int]) -> int:
