...
Your task is to return all possible valid IP addresses that can be created by inserting exactly three dots into the string. 
You must not reorder or remove any digits.
...
Нужно вернуть все возможные корректные IP-адреса, которые можно получить, вставляя ровно три точки в строку. 
Нельзя менять порядок символов или удалять их.
