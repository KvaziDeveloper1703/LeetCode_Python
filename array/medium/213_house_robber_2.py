...
Given an integer array numbers where numbers[i] is the amount of money in the i-th house.
...
Вы — профессиональный грабитель, планирующий ограбление домов на улице. 
    + В каждом доме спрятано некоторое количество денег. 
    + Все дома расположены по кругу, то есть первый и последний дома являются соседями.
    + Вы не можете ограбить два соседних дома, иначе сработает система безопасности и вызовет полицию.

Дан массив целых чисел numbers, где numbers[i] — количество денег в i-ом доме. 

Верните максимальную сумму денег, которую можно ограбить без срабатывания сигнализации.
...
def rob(numbers: List[int]) -> int:
    ...
    total_houses = len(numbers)
    if total_houses == 0:
        return 0
    if total_houses == 1:
        return numbers[0]

    max_rob_excluding_first = rob_linear(numbers[1:])
    max_rob_excluding_last = rob_linear(numbers[:-1])
