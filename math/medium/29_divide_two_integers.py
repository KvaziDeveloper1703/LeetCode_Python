...
The function should return the quotient after dividing dividend by divisor. However, if the result of the division exceeds the range of a 32-bit signed integer, which is from -2³¹ to 2³¹ − 1, then you must return 2147483647 if the result is too large, or -2147483648 if it is too small.
...
Функция должна вернуть частное от деления dividend на divisor. Если результат выходит за пределы допустимого диапазона 32-битного целого числа со знаком (от -2³¹ до 2³¹ − 1, то есть от -2147483648 до 2147483647), то необходимо вернуть 2147483647, если результат превышает предел, или -2147483648, если он меньше допустимого значения.
